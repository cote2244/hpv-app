...
# --- 2. หมวดข้อมูล: ความเสี่ยงสูง (HPV 16 / 18) ---
data_high_risk = {
...
# --- 3. หมวดข้อมูล: สายพันธุ์เสี่ยงอื่นๆ ---
data_other_risk = {
    "result_type": "HPV 6,11,26,31,33,35,39,42,43,44,45,51,52,53,56,58,59,66,68,73,82",
    "risk_level": "⚠️ ต่ำ",
...
mock_db = {
    ...
    "POSITIVE HPV 16": data_high_risk,
    
    "HPV 18": data_high_risk,
    "HPV18": data_high_risk,
    "POSITIVE HPV 18": data_high_risk,
}

# คำสั่งเพิ่มรหัสสายพันธุ์อื่นๆ เข้าไปในระบบค้นหาอัตโนมัติ (เช่น HPV 31, HPV 52 ฯลฯ)
other_strains = ["6", "11", "26", "31", "33", "35", "39", "42", "43", "44", "45", "51", "52", "53", "56", "58", "59", "66", "68", "73", "82"]
for strain in other_strains:
    mock_db[f"HPV {strain}"] = data_other_risk
    mock_db[f"HPV{strain}"] = data_other_risk
